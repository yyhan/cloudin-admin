...
<div class="wrapper">
                ...
                <#if menu?? && menu.menuItems??>
                    <#list menu.menuItems as menuItem>
                        <#if menuItem.subMenus??>
                            <li class="treeview">
                                <a href="${menuItem.url!}"><i class="${menuItem.icon!"fa fa-circle"}"></i> <span> ${menuItem.name}</span>
                                    <span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span>
                                </a>
                                <ul class="treeview-menu">
                                    <#list menuItem.subMenus as subMenuItem>
                                        <li><a href="${subMenuItem.url!}"><i class="${subMenuItem.icon!"fa fa-circle"}"></i> ${subMenuItem.name}</a></li>
                                    </#list>
                                </ul>
                            </li>
                        <#else>
                             <li><a href="${menuItem.url!}"><i class="${menuItem.icon!"fa fa-circle"}"></i> ${menuItem.name}</a></li>
                        </#if>
                    </#list>
                </#if>
            </ul>
    ...
    <footer class="main-footer">
        ...
        <strong>Copyright &copy; 2018 <a href="http://www.jubaopay.com">凡伟网络科技有限公司</a>.</strong>
    </footer>
...
<script type="text/javascript">
    $(document).ajaxComplete(function (event, xhr, options) {
        ...
        if (xhr.status == 302) {
            ...
            // 未登录错误码和重复登录，统一处理
            if (responseJson.code == 401 || responseJson.code == 10099) {
